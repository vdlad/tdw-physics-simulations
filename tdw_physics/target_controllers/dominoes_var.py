...
class Dominoes(RigidbodiesDataset):
    ...
    def __init__(self,
        ...
        self.camera_aim = {"x": 0., "y": 0.5, "z": 1} # fixed aim
    ...
    def element_wise_equal(self, x, y):
        for ele in x:
            if x[ele] != y[ele]:
                return False
        return True
    def _place_second_target_zone(self) -> List[dict]:
        # create a target zone (usually flat, with same texture as room)
        if (self.zone_color == [1.,1.,0.]).all():
            zone_rgb = [1.,1.,1.]
        else:
            zone_rgb = [1.,1.,0.]
        #Vedang - want to make it the right height
        ...
        commands = []
        #Vedang - Paint a target on the base
        rot = copy.deepcopy(TDWUtils.VECTOR3_ZERO)
        rot['z'] = 90.
        if self.add_second_ramp:
            commands.extend(
                self.add_primitive(
                    record=record,
                    position=(self._get_zone_second_location(scale)),
    ...
    def _place_target_zone(self) -> List[dict]:
        #vedang - making the first one sliightly lower than first 
        shortened_scale = copy.deepcopy(self.zone_scale_range)
        shortened_scale['y']=shortened_scale['y']/2

        # create a target zone (usually flat, with same texture as room)
        if not self.repeat_trial: # sample from scratch
            
            record, data = self.random_primitive(self._zone_types,
                                                 scale=shortened_scale,
            ...
            self.zone_color = rgb
            self.zone_id = o_id
            self.zone_scale = shortened_scale
        else:
            # dry pass to get the obj id counter correct
            record, data = self.random_primitive([self.zone],
                                                 scale=shortened_scale,
        ...
        # place it just beyond the target object with an effectively immovable mass and high friction
        commands = []
        commands.extend(self._place_second_target_zone())
        commands.extend(
            self.add_primitive(
                record=record,
                ...
                scale_mass=False,
                dynamic_friction=self.zone_friction,
                static_friction=(self.zone_friction+0.02),
                bounciness=0,
                o_id=o_id,
                add_data=(not self.remove_zone),
                make_kinematic=True # zone shouldn't move
    ...
    def _add_ramp_base_to_ramp(self, color=None) -> None:
        #Vedang - I added in the rotation to make it the same as the rotatated ramp
        cmds = []
        ...
        self.ramp_base_scale = arr_to_xyz([
            ...
            float(self.ramp_scale['z'] * 0.95*r_dep)]) #Vedang - it was getting stuck on the base edge
        ...
        cmds.extend(
            RigidbodiesDataset.add_physics_object(
                ...
                # rotation=TDWUtils.VECTOR3_ZERO,
                rotation=self.ramp_rot, #Vedang Here
