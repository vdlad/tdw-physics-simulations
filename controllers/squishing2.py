...
class Squishing(FlexDataset):
    ...
    def _get_squishable(self, position: Dict[str, float], rotation: [str, float]) -> \
        ...
        # Add the soft-body (squishable) object
        record = random.choice([f for f in self.records if f.name=="sphere"])
...
if __name__ == "__main__":
    ...
    Squishing().run(num=args.num, output_dir=args.dir, temp_path=args.temp, width=args.width, height=args.height, save_movies=True, save_passes=['_img'])
